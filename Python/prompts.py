"""Шаблоны промптов и описания инструментов для создания deep agents с нуля.

Этот модуль содержит все системные промпты, описания инструментов и шаблоны инструкций,
...
# === ОПИСАНИЕ ИНСТРУМЕНТА: ПРОСМОТР ФАЙЛОВОЙ СИСТЕМЫ (ls) ===
...
# === ОПИСАНИЕ ИНСТРУМЕНТА: ЧТЕНИЕ ФАЙЛА ===
...
# === ОПИСАНИЕ ИНСТРУМЕНТА: ЗАПИСЬ ФАЙЛА ===
WRITE_FILE_DESCRIPTION = """Create a new file in Russian or completely overwrite an existing file in Russian  in the virtual filesystem.
...
# === ШАБЛОН ПРОМПТА: СОЗДАНИЕ КРАТКОГО РЕЗЮМЕ РЕЗУЛЬТАТОВ ПОИСКА ===
# Используется для автоматической обработки найденных веб-страниц.
SUMMARIZE_WEB_SEARCH = """You are creating a minimal summary in Russian for research steering - your goal is to help an agent know what information it has collected, NOT to preserve all details.
...
- **Simple queries**: Use 5-7 search tool calls maximum
- **Normal queries**: Use 6-9 search tool calls maximum
- **Very Complex queries**: Use up to 10 search tool calls maximum
...
# === ПРЕФИКС ДЛЯ ДЕЛЕГИРОВАНИЯ ЗАДАЧ sub-agents ===
TASK_DESCRIPTION_PREFIX = """Delegate a task to a specialized sub-agent with isolated context. Available agents for delegation are:
{other_agents}
"""

#=== ИНСТРУКЦИИ ДЛЯ КООРДИНАТОРА sub-agents ===
